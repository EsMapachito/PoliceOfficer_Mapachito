			...
			for k,v in pairs(createdped) do
					if IsEntityDead(v) then
						if count[k] ~= nil then
							...
							count[k] = nil
							if x == 0 then
							TriggerServerEvent('mapachito:AddSomeMoney')
...
Citizen.CreateThread(function()
    while true do
		...
		local betweencoords = GetDistanceBetweenCoords(coords, 2704.65, -1407.18, 47.0, true)
		if betweencoords < 2.0 and not already then
			...
			if IsControlJustPressed(2, 0xCEFD9220) and not pressing then
				pressing = true
				GiveWeaponToPed_2(playerPed, 0x63F46DE6, 10, true, true, 1, false, 0.5, 1.0, 1.0, true, 0, 0)
				SetPedAmmo(playerPed, 0x63F46DE6, 100) -- springfield
				GiveWeaponToPed_2(playerPed, 0x7A8A724A, 1, true, true, 1, false, 0.5, 1.0, 1.0, true, 0, 0)
				SetPedAmmo(playerPed, 0x7A8A724A, 1) -- lazo
				SetPlayerModel(ped, 0xA9AD1A7D, true) -- policia casco
				missionstart()
				startscenario()
				already = true
			end
...
Citizen.CreateThread(function()
	Wait(0)
	Citizen.InvokeNative(0x554d9d53f696d002, -89429847, 2704.65, -1407.18, 47.0)
end)